...
// Hardcoded Exact TBA Addresses from verified screenshots
let tierBenchmarks = [
...
const WEB3_CONFIG = {
    TOKENS: [
        { symbol: "WETH", address: "0x0bd7d308f8e1639fab988df18a8011f41eacad73", priceUsd: 1900.00 }, // WETH drop contract
...
async function run() {
    console.log("Starting sync with verified TBA addresses...");
    ...
    try {
        ...
        // Peg WETH price to live ETH price
    ...
    // 2. Scan TBA Transferred Tokens & ETH
    for (let bm of tierBenchmarks) {
        ...
        for (let action of actions) {
            ...
            const url = `${BLOCKSCOUT_BASE_URL}?module=account&action=${action}&address=${tbaAddress}&page=1&offset=1000&sort=desc&apikey=${BLOCKSCOUT_API_KEY}`;
            const data = await secureFetch(url);

            if (data && data.status === "1" && Array.isArray(data.result)) {
                for (const tx of data.result) {
                    
                    // Filter for inbound transfers within the last 7 days
                    if (parseInt(tx.timeStamp, 10) >= sevenDaysAgo && tx.to && tx.to.toLowerCase() === tbaAddress && (!tx.isError || tx.isError === "0")) {

...
// FIX: Searches from Block 0 to capture every single historical mint and log perfectly
async function fetchAllLogs(address, topic0 = null) {
  ...
  while (fromBlock <= latestBlock) {
    ...
    // Automatically dynamically shrinks search window if logs are dense
...
async function fetchActivations() {
  ...
  const finalGross = []; // FIX: Tracks pure activations specifically for Burn Chart
  ...
  for (const dateStr of sortedDates) {
      ...
      runningGross += d.activated; // Gross never decreases
  ...
  return { 
    ...
    history: {
      ...
      cumulativeGross: finalGross // Sent to index.html to fix Burn chart
    },
    dualBurn
  };
}

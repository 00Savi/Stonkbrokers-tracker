 ...
 */
async function fromGgIndex(payload, { ethPriceUsd, tokenPrices = {}, lookbackDays = 7, rpc } = {}) {
  ...
  for (const v of listed) {
    if (v.quote_token) tokenSet.add(String(v.quote_token).toLowerCase());
    if (v.base_token) tokenSet.add(String(v.base_token).toLowerCase());
    if (v.token0) tokenSet.add(String(v.token0).toLowerCase());
    if (v.token1) tokenSet.add(String(v.token1).toLowerCase());
  }
  ...
  for (const row of listed) {
    const quote = row.quote_token && String(row.quote_token).toLowerCase();
    const dec = decimals.get(quote) ?? 18;
    const px = pxOf(quote) || (quote === USDG ? 1 : 0);
    const tvlUsd = +asUsd(row.tvl_quote, dec, px).toFixed(2);
    const f = row.fees || {};
    const t0 = row.token0 && String(row.token0).toLowerCase();
    const t1 = row.token1 && String(row.token1).toLowerCase();
    const generated =

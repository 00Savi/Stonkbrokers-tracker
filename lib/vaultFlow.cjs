...
//
// 24h/7d/30d ACT is seeded from the last NFT transfer of each currently
// vaulted token (the buyer activated after that transfer). Hourly diffs of
// activations(id) still record in-place stakes and vault exits.
const fs = require("fs");
...
function emptyStats(liveBreakdown) {
  ...
  for (const t of TIERS) {
    stats[t] = {
      ...
      allTime: { act: Number(liveBreakdown?.[t] || 0), deact: 0, up: 0 },
    };
  }
  return stats;
}
...
function statsFromFlow(flow, liveBreakdown, now) {
  ...
  for (const e of flow) {
    ...
    if (e.type === "act") {
      ...
      // A level increase is not a sale. Count it as act at the new tier and
      // up (not deact) at the old one.
      if (tierStats[e.from]) {
        if (age <= oneDay) tierStats[e.from]["24h"].up++;
        if (age <= 7 * oneDay) tierStats[e.from]["7d"].up++;
        if (age <= 30 * oneDay) tierStats[e.from]["30d"].up++;
        tierStats[e.from].allTime.up++;
      }
      if (age <= oneDay) tierStats[t]["24h"].act++;
      if (age <= 7 * oneDay) tierStats[t]["7d"].act++;
      if (age <= 30 * oneDay) tierStats[t]["30d"].act++;
 ...
 */
function observe({ projectKey, prevSet, prevFlow, seedEvents, currSet, liveBreakdown, liveCount, now }) {
  const ts = now || Math.floor(Date.now() / 1000);
  const disk = load();
  const added = diffSets(prevSet, currSet, ts);
  const flow = prune(
    mergeEvents(mergeEvents(disk[projectKey], prevFlow), seedEvents).concat(added),
    ts,
  );

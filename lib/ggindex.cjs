...
class GgIndex {
  ...
  async get(path, params = {}, callOpts = {}) {
    const url = new URL(this.base + path);
    for (const [k, v] of Object.entries(params)) {
      if (v !== undefined && v !== null) url.searchParams.set(k, String(v));
    }

    const timeoutMs = callOpts.timeoutMs ?? this.timeoutMs;
    const attempts = callOpts.attempts ?? this.attempts;
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      try {
        ...
        // 404/400 understood the question and the answer will not change this
        // run. 409 is "fold not ready yet" -- that CAN clear during a job if
        // we wait, and failing immediately is what made hourly runs look
        // intermittent when the indexer was a minute behind head.
        ...
        if (res.status === 409) {
          const detail = String(body?.detail || body?.error || "not ready");
          // A token that has never been backfilled will still be 409 on the
          // next attempt. Waiting 8s x 6 burned minutes on INDEX/PRINTER/OAKMONT.
          if (/backfill|not indexed|unknown token|no historical/i.test(detail)) {
            throw new Error(`gg-index 409 ${url.pathname}: ${detail}`);
          }
          lastErr = `HTTP 409 ${url.pathname}: ${detail}`;
        ...
        lastErr = e.name === "TimeoutError" || e.name === "AbortError" ? "timeout" : e.message;
      }
    }

    throw new Error(`gg-index unreachable after ${attempts} attempts (${url.pathname}): ${lastErr}`);
  }
  ...
  async supplies(addresses) {
    ...
    // Cap 50 is the server's. Chunk smaller so one slow batch cannot stall the
    // whole meme+stock list, and give up faster than the default 45s×4 — that
    // wait is what made the hourly job die before any project was rebuilt.
    for (let i = 0; i < addresses.length; i += 15) {
      ...
      try {
        ...
        // A missing chunk leaves those tokens absent from the map. Callers
        // already treat a missing supply as unknown (meme list falls back to
        // 1e9; protocol tokens carry the previous snapshot). Throwing here is
        // what aborted the hourly job before any project was rebuilt.
        console.warn(`[warn] supplies chunk ${chunk[0]}…: ${e.message}`);
      }
    }
    return out;
  }
  ...
  /** UTC-day native ETH inflow at an address. Use `inflow_usd` for revenue, not `net_*`. */
  async nativeFlowsDaily(address, { days = 14 } = {}) {
    return this.get(`/v1/addresses/${address}/native_flows/daily`, { days });
  }
  ...
  /**
   * Safety Deposit Box Smart LP vaults and FeesCollected in the lookback.
   *
   * Amounts are base-unit strings. The fetcher prices them. Throws on 404
   * (catalog not seeded) the same way other reads do — the caller falls back
   * to the chain walk rather than publishing an empty vault list as fact.
   */
  async smartLps(slug, { days = 7, timeoutMs, attempts } = {}) {
    return this.get(`/v1/projects/${slug}/smart_lps`, { days }, { timeoutMs, attempts });
  }

  /**
   * Daily protocol revenue streams (AMM, Clock-In, launch volume, tax, DEX).
   *
   * Each cell is `{ usd, source, complete }` or null. Missing days are omitted.
   * `source` is chain, native, or git. Oracle-wallet AMM from chain/native is
   * the unscaled sample; the fetcher applies the oracle-weight scale.
   */
  async revenueDaily(slug, { days = 120 } = {}) {
    return this.get(`/v1/projects/${slug}/revenue/daily`, { days });
  }

  /**
   * Daily supply / burn from the token's first mint, folded from Transfers.
   *
   * `burned` is token units (cap − supply + dead + zero + activation lock).
   * Empty `daily` means the index has not rebuilt supply_days yet.
   */
  async projectSupplyDaily(slug, { days = 365 } = {}) {
    return this.get(`/v1/projects/${slug}/supply/daily`, { days });
  }
}

...
function daySteps(summary) {
  let best = 0;
  const series = [summary?.daily?.wallets];
  for (const row of Object.values(summary?.byProject || {})) series.push(row?.daily?.wallets);
...
/**
 * A partial rescan can stamp a whole project onto one day and replace a ramp
 * with a flat line. Keep the published history until the new fold has a
 * comparable shape.
 */
function keepOnboardingHistory(prev, next) {
  if (!next) return prev || next;
  if (!prev || !(Number(prev.wallets) > 0)) return next;
  const prevSteps = daySteps(prev);
  const nextSteps = daySteps(next);
  const prevW = Number(prev.wallets) || 0;
  const nextW = Number(next.wallets) || 0;
  if (prevSteps >= 8 && nextSteps * 3 < prevSteps && nextW <= prevW) {
    console.warn(
      `[warn] onboard: new series collapsed (${nextW} wallets, ${nextSteps} steps) vs published (${prevW}, ${prevSteps}); keeping the published history`,
    );
    return {
      ...prev,
      pending: Number.isFinite(Number(next.pending)) ? next.pending : prev.pending,
      complete: false,
    };
  }
  return next;

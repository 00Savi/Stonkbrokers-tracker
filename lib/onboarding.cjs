...
function seriesSteps(summary) {
  const out = { all: new Set(summary?.daily?.wallets || []).size };
  for (const [key, row] of Object.entries(summary?.byProject || {})) {
    out[key] = new Set(row?.daily?.wallets || []).size;
  }
  return out;
}

/**
 * A partial rescan publishes a lower total and a nearly flat line over the
 * ramp. Keep the published history until the new fold is finished and still
 * has the shape.
 */
function keepOnboardingHistory(prev, next) {
  if (!next) return prev || next;
  if (!prev || !(Number(prev.wallets) > 0)) return next;
  const prevW = Number(prev.wallets) || 0;
  const nextW = Number(next.wallets) || 0;
  const pending = Number(next.pending) || 0;
  const prevS = seriesSteps(prev);
  const nextS = seriesSteps(next);
  const lostRamp = Object.keys(prevS).some((key) => prevS[key] >= 8 && (nextS[key] || 0) * 2 < prevS[key]);
  const partial = pending > 0 && nextW < prevW;
  if (lostRamp || partial) {
    console.warn(
      `[warn] onboard: keeping published history (${prevW} wallets) over ${nextW} wallets / ${nextS.all} steps` +
        (partial ? " while the scan is still pending" : " because a ramp collapsed"),
    );
    return {
      ...prev,
      pending,

...
const { utcIso, utcIsoFromTs, dateKey } = require("./dates.cjs");
...
function chartFromLlama(payload, days = 365) {
...
function snapshotPush(prev, row) {
  ...
  for (const s of list) {
    if (s?.date) byDate.set(dateKey(s.date), s);
  }
  byDate.set(dateKey(row.date) || row.date, { ...row, date: dateKey(row.date) || row.date });
  const out = [...byDate.values()].sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
  if (out.length > 90) out.splice(0, out.length - 90);
  return out;
}

/** One point per UTC day. Same-day reruns overwrite the last sample. */
function appendDaily(series, dateStr, value) {
  const labels = Array.isArray(series?.labels) ? [...series.labels] : [];
  const data = Array.isArray(series?.data) ? [...series.data] : [];
  const n = Number(value);
  if (!Number.isFinite(n) || n <= 0) return { labels, data };
  if (labels[labels.length - 1] === dateStr) data[data.length - 1] = n;
  else {
    labels.push(dateStr);
    data.push(n);
  }
  if (labels.length > 90) {
    const drop = labels.length - 90;
...
async function supplyBurn(gg, tokenCa, maxSupply) {
  ...
  const circulating = Math.max(0, currentSupply - dead);
  return { currentSupply, circulating, totalBurnTokens, deadBalance: dead };
}
 ...
 */
async function fetchSpecialBurnHistory(gg, key, previous) {
  if (!gg?.projectSupplyDaily) return previous && previous.labels?.length ? previous : null;
  try {
...
async function buildSpecialProject({ key, conf, market, prev, gg, dexPairs }) {
  ...
  try {
    ...
    if (prevCirc > 0) {
      burn = {
        currentSupply: prevCirc,
        circulating: prevCirc,
        totalBurnTokens: prevBurn || 0,
        deadBalance: 0,
      };
    } else {
      burn = {
        currentSupply: conf.maxSupply,
        circulating: conf.maxSupply,
        totalBurnTokens: 0,
        deadBalance: 0,
      };
    }
  ...
  if (conf.llamaSlug) {
    try {
      ...
      cashflow = {
        ...
        dailyDates: chartFromLlama(fees, 365).labels,
        dailyFees: chartFromLlama(fees, 365).values,
        dailyRevenue: chartFromLlama(holders || revenue, 365).values,
      };
    } catch (e) {
  ...
  let circulatingSupply = burn.circulating ?? Math.max(0, (burn.currentSupply || 0) - (burn.deadBalance || 0));
  let fdv = circulatingSupply * (market.tokenPriceUsd || 0);
  let annualYield = 0;
  let eligible = prev?.activation?.eligibleWallets ?? null;
  let activeCount = null;
  let totalSupplyUnits = conf.nftCa ? conf.collectionSupply || 0 : circulatingSupply;
  ...
  if (key === "index") {
    ...
    if (!tokenHolders) {
      const gecko = await fetchGeckoHolders(conf.tokenCa);
      if (gecko) tokenHolders = gecko;
    }
    if (!tokenHolders && prev?.ownership?.tokenHolders) tokenHolders = prev.ownership.tokenHolders;
    // Eligible stays null on a 409 rather than becoming 0. compactNum shows —.
    activeCount = eligible;
    totalSupplyUnits = tokenHolders;
  }
  ...
  if (key === "coattail") {
    ...
    try {
      ...
      if (coattail.activationBurnCoat > 0 && burn.totalBurnTokens < coattail.activationBurnCoat) {
        ...
        burn.circulating = Math.max(0, burn.currentSupply - (burn.deadBalance || 0));
      }
    } catch (e) {
  ...
  if (key === "oakmont") {
    ...
    try {
      oakmontVault = await fetchOakmontVault(conf.reserveCa);
      ...
      market.vaultSource = oakmontVault.source || "oakmont-api";
      if (!(market.vaultNavUsdg > 0) && oakmontVault.oakSupply > 0 && market.tokenPriceUsd > 0) {
        // Indexer NAV is the basket in USDG. When it is down, escrowed $STRIKE
        // inside the ERC-4626 wrapper is the on-chain backing we can prove.
        market.vaultNavUsdg = oakmontVault.oakSupply * market.tokenPriceUsd;
        market.vaultNavKind = "wrapped-strike";
      } else {
        market.vaultNavKind = "indexer";
      }
      market.navToFdv = fdv > 0 ? market.vaultNavUsdg / fdv : 0;
      ...
      const hasRev = Array.isArray(oakmontVault.revenue) && oakmontVault.revenue.length > 0;
      if (hasRev) {
        ...
        cashflow = {
          ...cashflow,
          source: oakmontVault.source || "oakmont-api",
          cadence: "month",
          ...
          dailyDates: oakmontVault.revenue.map((r) => {
            const s = String(r.date || "");
            return /^\d{4}-\d{2}-\d{2}/.test(s) ? s.slice(0, 10) : (utcIsoFromTs(s) || dateKey(s));
          }),
          dailyFees: oakmontVault.revenue.map((r) => r.eth * ethPx),
          dailyRevenue: oakmontVault.revenue.map((r) => r.eth * ethPx),
        };
      } else if (prev?.cashflow?.dailyDates?.length) {
        cashflow = { ...prev.cashflow, source: prev.cashflow.source || "oakmont-api" };
        annualYield = cashflow.feesAnnualized || cashflow.revenueAnnualized || 0;
      } else {
        cashflow = { ...cashflow, source: oakmontVault.source || "onchain-erc4626", cadence: "month" };
      }
    } catch (e) {
  ...
  circulatingSupply = burn.circulating ?? Math.max(0, (burn.currentSupply || 0) - (burn.deadBalance || 0));
  fdv = circulatingSupply * (market.tokenPriceUsd || 0);
  const circulating = circulatingSupply || 1;
  ...
  const holdersNow = market.strikeHolders ?? tokenHolders;
  const dailySnapshots = snapshotPush(prev?.dailySnapshots, {
    ...
    tokenHolders: holdersNow || 0,
    nftHolders: nftHolders || 0,
    ownershipRatio: 0,
    activeCount: activeCount || 0,
    lockedStonk: lp.totalStonkLocked || 0,
    lockedLpUsd: lp.totalLpUsd || 0,
    fdv,
    annualYield,
    roi: roiPct,
    tiers: tiers.map((t) => ({
  ...
  return {
    ...
    ownership: {
      tokenHolders: holdersNow,
      stonkHolders: tokenHolders,
      nftHolders: nftHolders,
      currentMaxSupply: conf.nftCa ? conf.collectionSupply : burn.currentSupply,
      circulatingNftSupply: conf.nftCa ? conf.collectionSupply : circulatingSupply,
      permanentlyBurntTokens: burn.totalBurnTokens,
      burntNfts: 0,
      ammVaultNfts: 0,
      ownershipRatio: 0,
      circulatingSupply,
      historicalGrowth: appendDaily(prev?.ownership?.historicalGrowth, todayStr, holdersNow),
      burnHistory: await fetchSpecialBurnHistory(gg, key, prev?.ownership?.burnHistory),
    },
    tiers,
    revenue: {
...
module.exports = { buildSpecialProject, isSpecial: (conf) => !!conf?.kind && conf.kind !== "nft" && conf.kind !== "factions" };

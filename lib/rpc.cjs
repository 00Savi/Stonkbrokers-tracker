...
const TRANSIENT =
  /EOF|timeout|timed out|deadline|too many requests|429|403|forbidden|408|500|502|503|504|connect|no route|ehostunreach|econnrefused|econnreset|temporarily|try again|reset by peer|unavailable|rate/i;
...
class Rpc {
  constructor(url = RPC_URL, opts = {}) {
    ...
    this.minSpacingMs = opts.spacingMs ?? 320;
    this.spacingMs = this.minSpacingMs;
    this._easeLogAt = this.spacingMs;
    this.id = 0;
  ...
  // A 429 must not freeze the rest of the job at 2s. After successes, walk
  // back toward the measured sustainable floor (320ms). Without this, one
  // Stonk getLogs burst made Card Wall's 4k eth_calls take ~15 minutes.
  ease() {
    if (this.spacingMs <= this.minSpacingMs) return;
    this.spacingMs = Math.max(this.minSpacingMs, this.spacingMs - 100);
    if (this.spacingMs <= this._easeLogAt - 400 || this.spacingMs === this.minSpacingMs) {
      this._easeLogAt = this.spacingMs;
      console.warn(`[warn] rpc recovered; spacing now ${this.spacingMs}ms`);
    }
  }
  ...
  async sendBatch(reqs) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      if (attempt) {
        const rate = /429|too many requests|403|forbidden/i.test(lastErr);
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 429 || res.status === 403) this.throttle(String(res.status));
          continue;
        }
        let parsed;
        try {
  ...
  /**
   * eth_getTransactionByHash, batched. Missing/failed hashes map to null.
   */
  async transactions(hashes) {
    const uniq = [...new Set((hashes || []).filter(Boolean).map((h) => String(h).toLowerCase()))];
    const out = new Map();
    for (let i = 0; i < uniq.length; i += this.batchSize) {
      if (i && this.spacingMs) await sleep(this.spacingMs);
      const chunk = uniq.slice(i, i + this.batchSize);
      const res = await this.sendBatch(
        chunk.map((h) => ({ method: "eth_getTransactionByHash", params: [h] })),
      );
      const byId = new Map(res.map((r) => [r.id, r.error ? null : r.result]));
      const ids = res.map((r) => r.id).sort((a, b) => a - b);
      ids.forEach((id, k) => out.set(chunk[k], byId.get(id) || null));
    }
    return out;
  }

  /**
   * eth_getCode, batched. Empty bytecode is an EOA.
   */
  async codes(addresses) {
    ...
    for (let i = 0; i < uniq.length; i += this.batchSize) {
      ...
      const byId = new Map(res.map((r) => [r.id, r.error ? "0x" : r.result]));
      const ids = res.map((r) => r.id).sort((a, b) => a - b);
      ids.forEach((id, k) => out.set(chunk[k], byId.get(id) || "0x"));
    }
    return out;
  }
}

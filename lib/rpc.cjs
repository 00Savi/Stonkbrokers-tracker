...
/** Errors meaning "you asked for too much", answered by SPLITTING the range.
 *
 *  The node has two entirely different ways of saying a log query is too big:
 *  it returns exactly 10,000 results and says nothing, or it returns an
 *  explicit `logs matched by query exceeds limit of 10000`. Only the silent
 *  form was handled, so the explicit one surfaced as a hard failure and killed
 *  the run — observed on the mancer NFT's 34,749-log history. Retrying a
 *  request that is too large will never succeed; narrowing is the only thing
 *  that helps, so the message has to be classified as such. */
const NARROWABLE =
  /exceeds limit|too many results|response size|query returned more than|limit of \d+/i;
const isNarrowable = (msg) => isTransient(msg) || NARROWABLE.test(String(msg));
    ...
    for (let attempt = 0; attempt < 7; attempt++) {
      ...
      try {
        ...
        const res = await fetch(this.url, {
          ...
          // Unbounded fetch is how a rate-limited node turns an hourly job
          // into a 6-hour GitHub Actions hang. The next cron then either
          // cancels this run (no commit) or queues behind it.
  ...
  async getLogs({ address, fromBlock, toBlock, topics }, onProgress) {
    ...
    while (from <= toBlock) {
      ...
      try {
        ...
        // isNarrowable, not isTransient: an explicit "exceeds limit" is a real
        // error message, so isTransient rejected it and threw. Leaving `logs`
        // null routes it into the halve-and-retry path below, which is the
        // only thing that can actually fix it.
        else if (!isNarrowable(r.error.message)) throw new Error(r.error.message);
      } catch (e) {
        if (!isNarrowable(e.message)) throw e;
      }

...
function slimPools(pools) {
  const out = {};
  for (const id of Object.keys(FACTION_TOKENS)) {
    out[id] = {
      weth: Number(pools?.[id]?.weth) || 0,
      usd: Number(pools?.[id]?.usd) || 0,
    };
  }
  return out;
}
...
function mergeHistory(chainRows, prevHistory, live) {
  ...
  return (chainRows || []).map((row) => {
    ...
    return {
      ...
      poolsWeth: current ? live.poolsWeth : (prev.poolsWeth ?? null),
      pools: current ? live.pools : (prev.pools || null),
    };
  });
}

/** One UTC-day stamp of the four faction pools. Same-day hourly runs overwrite. */
function mergeLpHistory(prevRows, live) {
  const date = utcDate(Math.floor(Date.now() / 1000));
  if (!date) return Array.isArray(prevRows) ? prevRows : [];
  const rows = (Array.isArray(prevRows) ? prevRows : []).filter((r) => r && r.date);
  const point = {
    date,
    nightId: live.nightId,
    totalWeth: Number(live.poolsWeth) || 0,
    totalUsd: Number(live.poolsWethUsd) || 0,
    pools: slimPools(live.pools),
  };
...
async function fetchNight(rpc, opts = {}) {
  ...
  return {
    ...
    history,
    lpHistory: mergeLpHistory(prev?.lpHistory, {
      nightId: status.nightId,
      poolsWeth,
      poolsWethUsd: usdOf(poolsWeth, ethPriceUsd),
      pools,

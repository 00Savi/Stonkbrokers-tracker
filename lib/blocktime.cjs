...
class BlockTime {
  ...
  at(block) {
    ...
    // A block older than the first anchor has no timestamp. Clamping it to
    // that anchor stamped every early wallet onto one day and flattened the
    // onboard chart.
    if (block < a[0][0]) return 0;

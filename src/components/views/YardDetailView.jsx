...
export default function YardDetailView({ data, activeTab }) {
  ...
  const revData1 = hasRevData ? revenue.dailyAmm : [15000, 16000, 15000, 18000, 16000, 17000, 15000];
  const revData2 = hasRevData && revenue.dailyDex?.length ? revenue.dailyDex : [3000, 4000, 3500, 3000, 4500, 3000, 3000];
  const revData3 = hasRevData && revenue.dailyLaunchpad?.length ? revenue.dailyLaunchpad : [6000, 6000, 7000, 6000, 6500, 6500, 7000];
  ...
  const actCum = (hasActHist && actHistory.cumulative?.length) ? actHistory.cumulative : [1300, 1320, 1350, 1380, 1390, 1405, 1420];
  ...
  const rawOwnData = hasOwnHist ? ownHistGrowth.data : [1300, 1310, 1330, 1350, 1370, 1380, 1390];
  ...
  let yardHolders = Number(ownership.yardHolders) || Number(ownership.tokenHolders) || Number(ownership.erc20Holders) || chartLastValue;
  if (yardHolders > 0 && yardHolders < chartLastValue * 0.7) yardHolders = chartLastValue;
    ...
    <div className="space-y-6 relative">
                      ...
                      <tr onClick={() => setExpandedTier(isExpanded ? null : t.tier)} className="hover:bg-[#334155]/20 transition cursor-pointer group">
                        <td className="py-5 pl-2">
                          <div className="flex items-center gap-3">
                            <span className="bg-[#0f172a] border border-[#334155] text-cyan-400 px-2.5 py-1 rounded text-xs font-bold shadow-inner">{t.tier}</span>
                            <div>
                            ...
                            <div className="relative h-32 md:h-40 w-full">
                              <Line 
                                data={{ 
                                  ...
                                  datasets: [{ label: 'Daily Yield (USD)', data: t.dailyYields?.length ? t.dailyYields : [12, 10, 18, 14, 20, 12, 18], borderColor: '#22d3ee', backgroundColor: 'rgba(34, 211, 238, 0.1)', borderWidth: 2, fill: true, tension: 0.4, pointRadius: 3 }] 
                                }} 
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-5 shadow-inner">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Protocol Vault Inflows (7D)</p>
              <p className="text-2xl font-extrabold text-emerald-400">{formatCurrency(revenue.ammFeesUsd || 0)}</p>
            </div>
            <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-5 shadow-inner">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Yard AMM Protocol Fees (7D)</p>
              <p className="text-2xl font-extrabold text-cyan-400">{formatCurrency(revenue.dexFeesUsd || 0)}</p>
            </div>
            <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-5 shadow-inner">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Ecosystem Inflows (7D)</p>
            ...
            <div className="relative h-72 md:h-80 w-full">
              <Bar 
                data={{
                  labels: revDates,
                  datasets: [
                    { label: "Vault Distributions", data: revData1, backgroundColor: "#34d399", borderRadius: 4 },
                    { label: "Yard AMM Fees", data: revData2, backgroundColor: "#22d3ee", borderRadius: 4 },
                    { label: "Ecosystem Fees", data: revData3, backgroundColor: "#a855f7", borderRadius: 4 }
                  ]
                }} 
            ...
            <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-4 md:p-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-2">
                ...
                <div className="text-right flex flex-col items-end w-full md:w-auto">
                  ...
                  <button onClick={() => setLpTableOpen(!lpTableOpen)} className="mt-2 text-[10px] bg-[#1e293b] border border-[#334155] text-slate-300 px-3 py-1 rounded hover:text-white transition shadow-sm w-full md:w-auto">
              ...
              {lpTableOpen && (
                <div className="overflow-x-auto transition-all duration-300">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead><tr className="text-slate-400 border-b border-[#334155]"><th className="pb-2 pl-2">Trading Pair</th><th className="pb-2">DEX Venue</th><th className="pb-2 text-right">Tokens Locked</th><th className="pb-2 text-right pr-2">Total Pool Liquidity</th></tr></thead>
                    <tbody className="divide-y divide-[#334155]/40 text-slate-200">
                      {lockedLp.pools.map((p, i) => (
                        <tr key={i} className="hover:bg-[#334155]/20"><td className="py-2.5 pl-2 font-bold text-white">{p.pairName}</td><td className="py-2.5 text-slate-400">{p.dex}</td><td className="py-2.5 text-right font-bold text-orange-400">{formatNumber(p.stonkAmount)}</td><td className="py-2.5 text-right pr-2">{formatCurrency(p.liquidityUsd)}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
          ...
          <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-4 md:p-6 mb-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16">
              <div className="relative h-64 md:h-72 w-full md:w-1/2 flex items-center justify-center">
                <Doughnut data={{ labels: tiers.map(t => t.name), datasets: [{ data: breakdownArr, backgroundColor: ['#22d3ee', '#34d399', '#f472b6', '#fbbf24', '#a78bfa'], borderWidth: 0 }] }} options={{ responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { display: false } } }} />
              </div>
              <div className="w-full md:w-1/2 flex flex-col gap-3">
                {tiers.map((t, idx) => (
                  <div key={t.tier} className="flex justify-between items-center bg-[#1e293b] p-3 rounded-lg border border-[#334155]">
                    <div className="flex items-center gap-3"><div className={`w-4 h-4 rounded-md ${['bg-[#22d3ee]', 'bg-[#34d399]', 'bg-[#f472b6]', 'bg-[#fbbf24]', 'bg-[#a78bfa]'][idx % 5]}`}></div><span className="text-sm font-bold text-slate-300">{t.tier}: {t.name}</span></div>
             ...
             <div className="relative h-72 md:h-80 w-full">
                <Bar 
                  data={{
                    labels: actLabels,
                    datasets: [
                      { type: 'line', label: 'Net Active Units', data: actCum, borderColor: '#22d3ee', backgroundColor: 'rgba(34, 211, 238, 0.05)', borderWidth: 3, fill: true, tension: 0.3, yAxisID: 'y' },
      ...
      {activeTab === 'ownership' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-5 shadow-sm"><p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Unique ${config.ticker} Holders</p><p className="text-2xl md:text-3xl font-extrabold text-purple-400">{formatNumber(yardHolders)} Wallets</p></div>
          </div>
          ...
          <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-4 md:p-6">
            ...
            <div className="relative h-72 md:h-80 w-full">
              <Line 
                data={{ labels: ownLabels, datasets: [{ label: 'Active Holders', data: ownData, borderColor: '#22d3ee', backgroundColor: 'rgba(34, 211, 238, 0.1)', borderWidth: 3, fill: true, tension: 0.3 }] }} 
      ...
      <div className="bg-[#1e293b] rounded-xl p-5 md:p-6 border border-[#334155] shadow-lg mt-8">
        <div className="flex items-center gap-2 mb-4">
          <svg className="w-5 h-5 text-cyan-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path></svg>

...
export default function YardDetailView({ data, activeTab }) {
  ...
  const { labels: revDates, cols: revCols } = sliceCols(
    rawRev.labels,
    [
      { ...(byKey.amm || { data: [] }), label: 'Vault Distributions', color: '#00a804' },
      { ...(byKey.dex || { data: [] }), label: 'Yard AMM Fees', color: '#38bdf8' },
      { ...(byKey.launch || { data: [] }), label: 'Ecosystem Fees', color: '#8b5cf6' },
    ],
    timeframe
  );
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-[#08090b] border border-[#1e2228] rounded-xl p-5 shadow-inner">
              ...
              <p className="text-2xl font-extrabold text-cyan-400">{formatCurrency(revCols[1]?.total || 0)}</p>
            </div>
            <div className="bg-[#08090b] border border-[#1e2228] rounded-xl p-5 shadow-inner">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Ecosystem Inflows ({revPeriod})</p>
              <p className="text-2xl font-extrabold text-purple-400">{formatCurrency(revCols[2]?.total || 0)}</p>
            </div>
          </div>
      ...
      <section id="ownership" className="scroll-mt-32">
        <div className="space-y-6">
          ...
          <div className="bg-[#08090b] border border-[#1e2228] rounded-xl p-4 md:p-6">
            ...
            <div className="relative h-52 sm:h-64 md:h-80 w-full">
              <Line 
                data={{ labels: ownLabels.slice(-ownN), datasets: [{ label: 'Active Holders', data: ownData.slice(-ownN), borderColor: '#38bdf8', backgroundColor: 'rgba(56, 189, 248, 0.1)', borderWidth: 3, fill: true, tension: 0.3 }] }} 

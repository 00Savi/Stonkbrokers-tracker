...
export default function MancerDetailView({ data, activeTab }) {
  ...
  const fullBurnData = Array.isArray(ownership.burnHistory) && ownership.burnHistory.length > 0 
    ? rightAlignArray(ownership.burnHistory, fullBurnLabels.length) 
    : rightAlignArray([20000000, 38000000, 55000000, 72000000, 84500000, 150000000, realBurntTokens], fullBurnLabels.length);
  ...
  // 4. Flywheel Chart
  const fwPrices = hasSnaps ? dailySnapshots.map(s => s.tokenPriceUsd || 0) : Array(masterDates.length).fill(0.0012);
  const fwBurn = hasSnaps ? dailySnapshots.map((s, i) => i === 0 ? 0 : Math.max(0, (s.totalBurn || 0) - (dailySnapshots[i-1].totalBurn || 0))) : Array(masterDates.length).fill(1500000);
                      ...
                      <tr onClick={() => setExpandedTier(isExpanded ? null : t.tier)} className="hover:bg-[#334155]/20 transition cursor-pointer group">
                        <td className="py-5 pl-2">
                          <div className="flex items-center gap-3">
                            <span className="bg-[#0f172a] border border-[#334155] text-purple-400 px-2.5 py-1 rounded text-xs font-bold shadow-inner">{t.tier}</span>
                            <div>
                            ...
                            <div className="relative h-32 md:h-40 w-full">
                              <Line 
                                data={{ 
                                  ...
                                  datasets: [{ label: 'Daily Yield (USD)', data: t.dailyYields?.length ? t.dailyYields : [10, 8, 15, 12, 18, 10, 15], borderColor: '#a855f7', backgroundColor: 'rgba(168, 85, 247, 0.1)', borderWidth: 2, fill: true, tension: 0.4, pointRadius: 3 }] 
                                }} 
            ...
            <div className="relative h-72 md:h-80 w-full">
              <Bar 
                data={{
                  labels: revDates,
                  datasets: [
                    { label: "DEX Swap Fees", data: revDataDex, backgroundColor: "#34d399", borderRadius: 4 },
                    { label: "Vault Inflows", data: revDataAmm, backgroundColor: "#a855f7", borderRadius: 4 },
                    { label: "Order Layer", data: revDataSec, backgroundColor: "#60a5fa", borderRadius: 4 }
                  ]
                }} 
             ...
             <div className="relative h-72 md:h-80 w-full">
                <Bar 
                  data={{
                    labels: actLabels,
                    datasets: [
                      { type: 'line', label: 'Net Active Units', data: actCum, borderColor: '#a855f7', backgroundColor: 'rgba(168, 85, 247, 0.05)', borderWidth: 3, fill: true, tension: 0.3, yAxisID: 'y' },
      ...
      {activeTab === 'ownership' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-5 shadow-sm"><p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Unique ${config.ticker} Holders</p><p className="text-2xl md:text-3xl font-extrabold text-purple-400">{formatNumber(mancerHolders)} Wallets</p></div>
          </div>
          ...
          <div className="bg-[#0f172a] border border-[#334155] rounded-xl p-4 md:p-6">
            ...
            <div className="relative h-72 md:h-80 w-full">
              <Line 
                data={{ labels: ownLabels, datasets: [{ label: 'Active Holders', data: ownData, borderColor: '#a855f7', backgroundColor: 'rgba(168, 85, 247, 0.1)', borderWidth: 3, fill: true, tension: 0.3 }] }} 
